...
function [B,norm_F] = Lap_jbfilter2(A,Guidance,w,sigma)
...
if size(A,3) == 1
   [B,norm_F] = bfltGray(A,Guidance,w,sigma(1),sigma(2));
else
   [B,norm_F] = bfltColor(A,Guidance,w,sigma(1),sigma(2));
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Implements bilateral filtering for grayscale images.
function [B,norm_F] = bfltGray(A,Guidance,w,sigma_d,sigma_r)
...
dim = size(A);
B = zeros(dim);norm_F = zeros(dim(1),dim(2));
for i = 1:dim(1)
   for j = 1:dim(2)
         ...
         I1 = Guidance(iMin:iMax,jMin:jMax);%%%%改的地方
      
         % Compute Gaussian intensity weights.
         H = exp(-(I1-Guidance(i,j)).^2/(2*sigma_r^2));%%%%改的地方
      
         % Calculate bilateral filter response.
         F = H.*G((iMin:iMax)-i+w+1,(jMin:jMax)-j+w+1);
         norm_F(i,j) = sum(F(:));
         B(i,j) = sum(F(:).*I(:))/norm_F(i,j);
...
function [B,norm_F] = bfltColor(A,Guidance,w,sigma_d,sigma_r)
...
for i = 1:dim(1)
   for j = 1:dim(2)
         ...
         jMax = min(j+w,dim(2));
         I = A(iMin:iMax,jMin:jMax,:);
         I1 = Guidance(iMin:iMax,jMin:jMax,:);
         ...
         F = H.*G((iMin:iMax)-i+w+1,(jMin:jMax)-j+w+1);
         norm_F(i,j) = sum(F(:));
         B(i,j,1) = sum(sum(F.*I(:,:,1)))/norm_F(i,j);
         B(i,j,2) = sum(sum(F.*I(:,:,2)))/norm_F(i,j);
         B(i,j,3) = sum(sum(F.*I(:,:,3)))/norm_F(i,j);
