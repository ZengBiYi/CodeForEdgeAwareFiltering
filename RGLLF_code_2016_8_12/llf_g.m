...
% f(x,ref)=x+fact*(I-ref)*exp(-(I-ref)虏/(2*sigma虏))
...
% Mathieu Aubry, Sylvain Paris, Samuel W. Hasinoff, Jan Kautz, and Fr茅do Durand. 
...
function [F]=llf_g(I,sigma,fact,N)
    [height width]=size(I);
    n_levels=ceil(log(min(height,width))-log(2))+2;%取整，大于本身的整数
    ...
    for ref=discretisation
        I_remap=fact*(I-ref).*exp(-(I-ref).*(I-ref)./(2*sigma*sigma));%将I换成G了
     % I_remap = G;
